...
public interface PatientMapper {
    ...
    @Select("SELECT patientId FROM slot_schedule WHERE ticket_number = #{ticketNumber}")
    int getPatientIdByTicketNumber(String ticketNumber);

    @Select("SELECT COUNT(id) FROM slot_schedule WHERE patient_id = #{patientId} date_id IN (SELECT id FROM date_schedule WHERE date >= #{startDate} AND date <= #{endDate}) AND ticket_number NOT LIKE 'C%'")
    int getPatientAppointmentsNumber(@Param("patientId")int patientId, @Param("startDate") String startDate, @Param("endDate") String endDate);
}
